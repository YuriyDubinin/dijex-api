...
| `POST` | `/api/servers/remote/images/purge` | Симметрично deploy: находит все контейнеры использующие `image:tag` на удалённом сервере, останавливает и удаляет их, затем удаляет сам образ (`docker rmi -f`). Финальная проверка, что ничего не осталось. Возвращает пошаговый отчёт (`steps[]`). |
